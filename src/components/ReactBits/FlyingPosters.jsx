...
class Canvas {
  ...
  next() {
    const step = this.medias[0] ? this.medias[0].height : 4;
  ...
  onTouchUp(e) {
    ...
    const dist = Math.hypot(endX - (this.startX || endX), endY - (this.start || endY));

    // If tap/click distance is small (not a drag), trigger onItemClick for the closest poster
    if (dist < 10 && this.medias && this.onItemClick) {
      let closest = null;
      let minDistance = Infinity;
      this.medias.forEach(media => {
        const d = Math.abs(media.plane.position.y);
        if (d < minDistance) {
          minDistance = d;
          closest = media;
        }
      ...
      if (closest && closest.index !== undefined) {
        this.onItemClick(closest.index);
      }
    }
  }
...
const FlyingPosters = forwardRef(function FlyingPosters({
  ...
  }, [items, planeWidth, planeHeight, distortion, scrollEase, cameraFov, cameraZ, onItemClick]);
