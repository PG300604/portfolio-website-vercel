...
export default function GridScan({
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const tiltSm = smoothDampFloat(tiltCurrent.current, tiltTarget.current, { v: tiltVel.current }, smoothTime, Infinity, dt);
      tiltCurrent.current = tiltSm.value;
      tiltVel.current = tiltSm.v;

      const yawSm = smoothDampFloat(yawCurrent.current, yawTarget.current, { v: yawVel.current }, smoothTime, Infinity, dt);
  ...
  }, [sensitivity, lineThickness, linesColor, scanColor, scanOpacity, gridScale, lineStyle, lineJitter, scanDirection, enablePost, noiseIntensity, bloomIntensity, scanGlow, scanSoftness, scanPhaseTaper, scanDuration, scanDelay, bloomThreshold, bloomSmoothing, chromaticAberration, smoothTime, skewScale, yBoost, tiltScale, yawScale]);

  return (
    <div ref={containerRef} className={`gridscan ${className}`} style={{ width: '100%', height: '100%', pointerEvents: 'none', ...style }} />
  );
}
